...
public class PropertyJsonInputTransformerTest {
  ...
  private static final String ID = "047e2b8db1fc4a68837e97a38f561d22";

  private static final Date DATE;

  static {
    try {
      DATE = parseDate("2018-03-21T15:33:51.341+0000");
    } catch (ParseException e) {
      throw new ExceptionInInitializerError("Failed to parse date string: " + e.getMessage());
    }
  }
  ...
  @Test
  public void testBasic() throws IOException, CatalogTransformerException {

    try (InputStream inputStream = getResource("/basic-example.json")) {

      AttributeInjectorImpl attributeInjector = new AttributeInjectorImpl(attributeRegistry);
      attributeInjector.setInjectableAttributes(injectableAttributes);

      PropertyJsonInputTransformer propertyJsonInputTransformer =
          new PropertyJsonInputTransformer(fallbackCommon, attributeInjector);
      ...
      assertThat(metacard.getCreatedDate(), is(DATE));
      assertThat(metacard.getTags(), is(new HashSet<>(Arrays.asList("resource", "VALID"))));
      assertThat(metacard.getResourceURI(), is(URI.create("content:" + ID)));
      ...
      assertValues(
          ...
          "https://localhost:8993/services/catalog/sources/ddf.distribution/"
              + ID
              + "?transform=resource");
      assertThat(metacard.getSourceId(), is("ddf.distribution"));
      assertValues(metacard, Core.METACARD_CREATED, DATE);
      assertThat(metacard.getEffectiveDate(), is(DATE));
      ...
      assertValues(metacard, Core.METACARD_MODIFIED, DATE);
  ...
  private <T> void assertValues(Metacard metacard, String attribute, T value) {
    assertThat(metacard.getAttribute(attribute), is(notNullValue()));
