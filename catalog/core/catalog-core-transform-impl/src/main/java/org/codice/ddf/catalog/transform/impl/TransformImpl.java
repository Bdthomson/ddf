...
public class TransformImpl implements Transform {
  ...
  public List<Metacard> transform(
    ...
    try (InputStream transformerStream =
      ...
      return metacards
          .stream()
          .peek(metacard -> setTitle(fileName, metacard))
  ...
  private <T> BinaryContent transformSourceResponse(
    ...
    if (CollectionUtils.isEmpty(transformers)) {
      throw new IllegalArgumentException(
          "Transformers mathcing " + identString + " " + id + " not found");
    }
    ...
    if (binaryContent == null) {
      throw new CatalogTransformerException(
          String.format(
              "Could not transform source response: " + identString + "=%s : %s",
              id,
  ...
  private List<Metacard> setMetacardIds(Supplier<String> idSupplier, List<Metacard> metacards) {
    metacards =
        metacards
            ...
            .peek(
                metacard ->
                    metacard.setAttribute(new AttributeImpl(Metacard.ID, calculateId(idSupplier))))
            .collect(Collectors.toList());
    return metacards;
  }
